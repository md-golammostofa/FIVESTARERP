...
namespace ERPWeb.Controllers
{
    ...
    public class ProductionController : BaseController
    {
        ...
        public ActionResult GetProductionAssembleItemStockInfo(string flag, long? floorId, long? modelId, long? warehouseId, long? itemTypeId, long? itemId, string lessOrEq, string reqNo, string status, string fromDate, string toDate, int page = 1)
        {
            if (string.IsNullOrEmpty(flag))
            {
                ViewBag.ddlLineNumber = _productionLineBusiness.GetAllProductionLineByOrgId(User.OrgId).Select(line => new SelectListItem { Text = line.LineNumber, Value = line.LineId.ToString() }).ToList();
        ...
        {
            var status = string.Format(@"'IMEI-Pass'");
        ...
        #region MiniStockTransferToWareHouse
        public ActionResult GetMiniStockTransferList(string flag)
        {
            if (string.IsNullOrEmpty(flag))
            {
                return View();
            ...
            else
            {
                ...
                List<HalfDoneStockTransferToWarehouseInfoViewModel> viewModels = new List<HalfDoneStockTransferToWarehouseInfoViewModel>();
                AutoMapper.Mapper.Map(dto, viewModels);
                return PartialView("_GetMiniStockTransferList", viewModels);
            }
        }

        public ActionResult SaveMiniStockTransferToSemiFinishGoodsWarehouse(MiniStockTransferToSemiFinishGoodsWarehouseViewModel model)
        {
            bool isSuccess = false;
            if (ModelState.IsValid)
            {
                MiniStockTransferToSemiFinishGoodsWarehouseDTO dto = new MiniStockTransferToSemiFinishGoodsWarehouseDTO();
                AutoMapper.Mapper.Map(model, dto);
                isSuccess = _miniStockTransferToSemiFinishGoodsWarehouseBusiness.SaveMiniStockTransferToSemiFinishGoodsWarehouse(dto, User.UserId, User.OrgId);
            }
            return Json(isSuccess);
        }
        ...
        #region Mini Stock Requisition To Warehouse For Semi Finish Goods
        public ActionResult CreateMiniStockRequisitionToWarehouseForSemiFinishGoods(string flag)
        {
            if (string.IsNullOrEmpty(flag))
            {
                ViewBag.ddlProductionFloor = _productionLineBusiness.GetAllProductionLineByOrgId(User.OrgId).Select(line => new SelectListItem { Text = line.LineNumber, Value = line.LineId.ToString() }).ToList();
        ...
        public ActionResult SaveMiniStockRequisitionToWarehouse(List<MiniStockRequisitionToSemiFinishGoodsWarehouseDetailViewModel> model, int totalQty)
        {
            bool isSuccess = false;
            if (ModelState.IsValid && totalQty > 0)
            {
                List<MiniStockRequisitionToSemiFinishGoodsWarehouseDetailDTO> dto = new List<MiniStockRequisitionToSemiFinishGoodsWarehouseDetailDTO>();
                AutoMapper.Mapper.Map(model, dto);
                isSuccess = _miniStockRequisitionToSemiFinishGoodsWarehouseInfoBusiness.SaveMiniStockRequisitionToWarehouse(dto, totalQty, User.UserId, User.OrgId);
            }
            return Json(isSuccess);
        }
        ...
        public ActionResult UpdateMiniWarehouseRequisitionStatus(long reqInfoId)
        {
            ...
            if (reqInfoId > 0)
            {
                isSuccess = _miniStockRequisitionToSemiFinishGoodsWarehouseInfoBusiness.UpdateMiniWarehouseRequisitionStatusForMiniWarehouseStock(reqInfoId, User.UserId, User.OrgId);
            }
            return Json(isSuccess);
        }
        #endregion

        #region Half-Done Transfer To Warehouse
        public ActionResult CreateHalfDoneProductTransferToWarehouse()
        {
            ViewBag.ddlProductionFloor = _productionLineBusiness.GetAllProductionLineByOrgId(User.OrgId).Select(line => new SelectListItem { Text = line.LineNumber, Value = line.LineId.ToString() }).ToList();
        ...
        public ActionResult SaveHalfDoneTransferToWarehouse(List<HalfDoneStockTransferToWarehouseDetailViewModel> model, int totalQty)
        {
            bool isSuccess = false;
            if (ModelState.IsValid && totalQty > 0)
            {
                List<HalfDoneStockTransferToWarehouseDetailDTO> dto = new List<HalfDoneStockTransferToWarehouseDetailDTO>();
                AutoMapper.Mapper.Map(model, dto);
                isSuccess = _halfDoneStockTransferToWarehouseInfoBusiness.SaveHalfDoneStockTransferToWarehouse(dto, totalQty, User.UserId, User.OrgId);
            }
            return Json(isSuccess);
        }
        ...
        public ActionResult UpdateHalfDoneTransferStatus(long transferInfoId)
        {
            bool isSuccess = false;
            if (transferInfoId > 0)
            {
                isSuccess = _halfDoneStockTransferToWarehouseInfoBusiness.UpdateHalfDoneTransferStatusForMiniStock(transferInfoId, User.UserId, User.OrgId);
            }
            return Json(isSuccess);
        }
        #endregion
        ...
        public ActionResult GetWeightCheck(string flag)
        {
            if (string.IsNullOrEmpty(flag))
            {
                return View();
        ...
        public ActionResult GetWeightCheckListByUserId(string imei, string fromDate, string toDate, int page = 1)
        {
            DateTime date = DateTime.Today;
            var dto = _weightCheckedIMEILogBusiness.GetAllWeightCheckedInfoByUserId(User.UserId, User.OrgId, date).Select(s => new WeightCheckedIMEILogDTO
            {
                WeightCheckedIMEILogId = s.WeightCheckedIMEILogId,
                CodeNo = s.CodeNo,
                IMEI = s.IMEI,
                EntryDate = s.EntryDate,
                EUserName = UserForEachRecord(User.UserId).UserName,
                ...
                ReferenceNumber = s.ReferenceNumber,
                Remarks = s.Remarks
            }).OrderByDescending(s => s.WeightCheckedIMEILogId).ToList();

            dto = dto.Where(s => (imei == null) || string.IsNullOrEmpty(imei) || s.IMEI.Contains(imei.Trim())).OrderByDescending(s => s.WeightCheckedIMEILogId).ToList();
            if (!string.IsNullOrEmpty(fromDate) || !string.IsNullOrEmpty(toDate))
            {
                dto = _weightCheckedIMEILogBusiness.GetAllDataByDateWise(fromDate, toDate, imei, User.UserId).OrderByDescending(s => s.WeightCheckedIMEILogId).OrderByDescending(s => s.WeightCheckedIMEILogId).ToList();
            }

            ViewBag.PagerData = GetPagerData(dto.Count(), 15, page);
            dto = dto.Skip((page - 1) * 15).Take(15).ToList();

            IEnumerable<WeightCheckedIMEILogViewModel> viewModel = new List<WeightCheckedIMEILogViewModel>();
            AutoMapper.Mapper.Map(dto, viewModel);

...
namespace ERPWeb.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        public ActionResult GetAssemblyLineWiseDashboardData(long assemblyId)
        {
            ...
            return Json(new { BrandName = tempData != null ? tempData.BrandName : "", ItemName = tempData != null ? tempData.ItemName : "", ModelName = tempData != null ? tempData.ModelName : "", ColorName = tempData != null ? tempData.ColorName : "", Time = DateTime.Now.ToString("hh:mm tt"), Date = DateTime.Now.Date.ToString("dd-MMM-yyyy"), LotIn = result.TotalLotIn, QCPass = result.TotalQCPass, Pending = (result.TotalLotIn - result.TotalQCPass) < 0 ? 0 : (result.TotalLotIn - result.TotalQCPass), QCFail = result.TotalQCFail, TotalQC1 = result.TotalQC1, TotalQC2 = result.TotalQC2, TotalQC3 = result.TotalQC3, RepairDone = result.TotalRepairDone, RepairPending = (result.TotalQC1 + result.TotalQC2 + result.TotalQC3 - result.TotalRepairDone) < 0 ? 0 : (result.TotalQC1 + result.TotalQC2 + result.TotalQC3 - result.TotalRepairDone), TotalItem = result.TotalHandset, TotalMiniWarehouseReceived = result.MiniStockReceivedQty });
        }
        public ActionResult GetPackegingLineWiseDashboardData(long packegingId)
        {
            ...
            return Json(new { BrandName = tempData != null ? tempData.BrandName : "", ItemName = tempData != null ? tempData.ItemName : "", ModelName = tempData != null ? tempData.ModelName : "", ColorName = tempData != null ? tempData.ColorName : "", Time = DateTime.Now.ToString("hh:mm tt"), Date = DateTime.Now.Date.ToString("dd-MMM-yyyy"), IMEIWrite = result.TotalIMEIWrite, Handset = result.TotalHandset, Pending = (result.TotalIMEIWrite - result.TotalHandset) < 0 ? 0 : (result.TotalIMEIWrite - result.TotalHandset), QCPass = result.TotalQCPass, BatteryWrite = result.TotalBatteryWrite, QCFail = result.TotalQCFail, RepairDone = result.TotalRepairDone, RepairPending = (result.TotalQCFail - result.TotalRepairDone) < 0 ? 0 : (result.TotalQCFail - result.TotalRepairDone), Carton = result.TotalCarton });
        }

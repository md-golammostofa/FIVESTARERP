...
{
   public class AccountsBaseRepository<T> : IBaseRepository<T> where T : class
    {
        private readonly IAccountsUnitOfWork _accountsUnitOfWork;
        internal DbSet<T> dbSet = null;
        public AccountsBaseRepository(IAccountsUnitOfWork accountsUnitOfWork)
        {
            if (accountsUnitOfWork == null) throw new ArgumentNullException("DbContext is not assigned");
            this._accountsUnitOfWork = accountsUnitOfWork;
            dbSet = this._accountsUnitOfWork.Db.Set<T>();
        }
        ...
        public bool Save()
        {
            ...
            return _accountsUnitOfWork.Db.SaveChanges() > 0;
        }

        public async Task<bool> SaveAsync()
        {
            return await _accountsUnitOfWork.Db.SaveChangesAsync() > 0;
        }
        ...
        {
            _accountsUnitOfWork.Db.Entry(entity).State = System.Data.Entity.EntityState.Modified;
        }
        ...
        {
            using (var cmd = _accountsUnitOfWork.Db.Database.Connection.CreateCommand())
            {
                cmd.CommandText = Sql;
